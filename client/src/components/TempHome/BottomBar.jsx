...
const BottomBar = ({ setPage, toys }) => {
  ...
  return (
    <>
      {map ? <div className='w-screen h-screen bg-slate-400/40 top-0 left-0 fixed z-30' onClick={() => {setMap(false); console.log('occured')}}></div> : null}
      <div className='fixed bottom-0 bg-white z-10 w-full shadow-md shadow-black rounded-tr-2xl rounded-tl-2xl flex justify-center space-x-14'>
        ...
        {!map && toys !== undefined ? <img data-testid='it-map' className='inline-block hover:opacity-30 w-9 h-9 translate-y-2' src='./icons/mapIcon.png' onClick={() => setMap(true)}/> : null}
      </div>
